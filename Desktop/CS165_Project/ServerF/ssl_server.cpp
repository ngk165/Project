...
//	SSL_CTX_set_options(ctx, SSL_OP_ALL | SSL_OP_NO_SSLv2 | SSL_OP_SINGLE_DH_USE);
	...
	char randchallenge[BUFFER_SIZE];
        bufflen = SSL_read(ssl,buff,BUFFER_SIZE);
	BIO *pkey= BIO_new_file("rsaprivatekey.pem","r");
    	RSA *decrypt = PEM_read_bio_RSAPrivateKey(pkey,NULL,NULL,NULL);

	int decryptlen=0;
    	decryptlen = RSA_private_decrypt(bufflen,(unsigned char*)buff,
	(unsigned char*)randchallenge,decrypt,RSA_PKCS1_PADDING);
	printf("DONE.\n");
	printf("    (Challenge: \"%s\")\n", buff2hex((const unsigned char*)randchallenge, 	decryptlen).c_str() );
	...
	BIO_write(file2,randchallenge,bufflen);
	...
	BIO * key = BIO_new_file("rsaprivatekey.pem","r");
	RSA *prkey = PEM_read_bio_RSAPrivateKey(key,NULL,NULL,NULL);
        while((reqfilelen = BIO_read(reqfile,fbuff,i))>1){
		int enclen = RSA_private_encrypt(reqfilelen,(unsigned char *)fbuff,
		(unsigned char*)fbuff2,prkey,RSA_PKCS1_PADDING);
       		 bytesSent += SSL_write(ssl,fbuff2,enclen);
	}
